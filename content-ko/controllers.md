...
라우트 `path`와 유사하게 `host` 옵션은 호스트 이름의 해당 위치에 있는 동적 값을 캡처하기 위해 토큰을 사용할 수 있습니다. 아래 `@Controller()` 데코레이터 예제에서 호스트 매개변수 토큰은 이러한 사용법을 보여줍니다. 이런 방식으로 선언된 호스트 매개변수는 메서드 시그니처에 추가되어야 하는 `@HostParam()` 데코레이터를 사용하여 접근할 수 있습니다.
